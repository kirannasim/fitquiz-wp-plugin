// simple_ira_calculator_form
if(jQuery('#simple_ira_calculator_form').length > 0){
    jQuery(document).ready(function ($j) {
        $j("#employee_switch").change(function () {
            var select = $j(this).find('input[name="employ_show"]:checked').val();
            if (select == 0) {
                $j(".ira_repeater").hide();
                $j(".add_employee").hide();
            } else {
    ...
    function ira_calculate() {
        var url = custom_vars.admin_url + "admin-ajax.php";
        var data = $j("#simple_ira_calculator_form").serialize();
        $j.ajax({
            url: url,
            type: "post",
            dataType: "json",
            ...
            success: function (response) {
                $j("#simple_ira_calculator_form").hide();
